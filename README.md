...
| AI       | Gemini, called from the Express server (`/api/assist/clarify` for ticket capture; Assist mode only for MVP) |
...
| `GEMINI_API_KEY`                      | Express server       | Powers `/api/assist/clarify` (conversational ticket capture). Server-side only. If unset, the new-ticket dialog returns 503 and the UI offers the manual form. |
...
src/
  components/
    ...
    tickets/         # TicketList, TicketDetailDialog (inline-edit),
                     # TicketCreateChat (LLM-driven capture, default),
                     # TicketCreateDialog (manual fallback / "Edit details"),
...
server/
  ...
  routes/
    ...
    assist-clarify.ts # POST /api/assist/clarify — Gemini-driven ticket capture
  lib/
...
- [x] Conversational ticket capture (Gemini asks up to 3 clarifying questions
  with prepopulated answer chips, then drafts the ticket; manual form remains
  as a fallback)
