...
src/
  components/
    ...
    tickets/         # TicketList, TicketDetailView (non-modal /loop/:shortId
                     #   page — inline-edit, hosts the assist panel),
                     # TicketTabsStrip (tabs above the detail view + ⌘O
                     #   jump-to-loop popover),
                     # TicketCreateInline (title-only quick capture — creates
                     #   the ticket and routes the user to /loop/:shortId),
                     ...
                     #   references — mounted in TicketDetailView),
  ...
  lib/
    ...
    ticketTabs.ts    # TicketTabsContext + useTicketTabs hook (open-loop
                     # tab state, keyed by short_id; consumed by
                     # TicketTabsStrip and TicketList)
    ticketTabsProvider.tsx # TicketTabsProvider component
    queries.ts       # useTickets*/usePeople hooks + createTicket/updateTicket
    supabase.ts      # typed Supabase client
    utils.ts         # cn() + small helpers
  pages/             # one file per view: Inbox, Now, Waiting, Follow-Up, Review, Stuck, People, Login, Loop (/loop/:shortId)
  types/
