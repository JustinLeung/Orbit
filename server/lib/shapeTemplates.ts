import type { Shape, ShapePhaseEntry } from './assistTypes.js'
...
// Templates pre-date the per-phase `action` field. Until each template
// gets a hand-authored action verb, default the action to the phase
// title — same fallback `appendPhaseToShape` uses for user-added phases.
// Refine turns will replace this with something concrete on the user's
// first interaction.
function phase(
  id: string,
  title: string,
